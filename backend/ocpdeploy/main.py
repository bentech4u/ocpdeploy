...
from .routers import auth as auth_router, imported as imported_router
...
_IMPORTED_ALLOWED = re.compile(r"^(|/health(/.*)?|/upgrade(/.*)?|/scale(/.*)?|/identity(/.*)?|/certs(/.*)?|/storage(/.*)?|"
                               r"/operators(/.*)?|/backup|/backup/run|/backup/download/[^/]+|/apps(/.*)?|/jobs(/.*)?|/status)$")
...
async def _access(request: Request, call_next):
    ...
    if m and imported.exists(m.group(1)):
        ...
        if c.get("read_only") and request.method != "GET":
